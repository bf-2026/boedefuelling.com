...
export default function Trust() {
  return (
    <section className="py-16 bg-white relative overflow-hidden border-t border-slate-200/50">
      {/* Background Decoration */}
      <div className="absolute inset-0 pointer-events-none">
        <div className="absolute top-20 left-0 w-96 h-96 bg-brand-primary/12 rounded-full blur-3xl opacity-50 mix-blend-multiply animate-blob" />
        <div className="absolute bottom-20 right-0 w-96 h-96 bg-brand-accent/12 rounded-full blur-3xl opacity-50 mix-blend-multiply animate-blob animation-delay-2000" />
      </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            {trustPoints.map((point, index) => (
              <motion.div
                ...
                className="p-6 bg-white/50 backdrop-blur-sm rounded-xl border border-slate-200/50 hover:border-brand-primary/30 hover:shadow-lg transition-all"
              >
